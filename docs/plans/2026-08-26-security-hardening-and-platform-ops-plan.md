...
**Backup destination REVISED (owner, 2026-08-31): OneDrive folder, not AWS S3.** The weekly sealed archive is published to a OneDrive-synced folder instead of S3. Invariant: **ciphertext only** — plaintext staging stays in the non-synced working folder; only the verified AES-256 `.7z` + manifest are copied into OneDrive. Recorded trade-offs vs the S3 design: no write-only-credential ransomware control (OneDrive sync propagates deletions; fallback = OneDrive versioning/recycle-bin — owner to confirm retention settings); Supabase **storage objects (~1.37 GB) lose their planned second copy** (platform backups cover the DB only) — open gap in the risk register; the S3 stage stays built and dormant in the scripts (one rclone config away). Microsoft becomes a holder of encrypted backup bytes → third-party record updated. The prior S3 design below is retained as the dormant option:
