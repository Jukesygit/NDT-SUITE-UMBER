...
**Publish-time screenshots, as implemented (2026-08-20).** The publisher's browser renders one offscreen frame per vessel and ships it as `vessels/<id>/screenshot.png`; no format change was needed. Four rulings hold it in shape:

- **The PII invariant extends to pixels.** The capture (`vessel-screenshot.ts`) sources its scene exclusively through `vessel-screenshot-state.ts`'s `screenshotSourceState`, which returns `sanitizeVesselStateForShare` output — never a caller's raw `VesselState`. A thumbnail cannot show what the JSON deliberately dropped. Pinned by `__tests__/vessel-screenshot-state.test.ts`, which needs no GPU.
- **The card mirrors the viewer's OPENING state by shared reference.** `SHARE_VIEWER_INITIAL_LAYERS` (in `bundle-types`) is what `ShareVesselViewer` seeds its layer state from and what the capture renders with — one constant, so the tile a client clicks and the view it opens cannot drift apart. It is empty, because exclusion is removal: an unpublished category has no entities left to hide.
- **Same engine as the viewer, not a second one.** The capture drives `SceneManager` + `buildReadOnlyScene` + `canonicalPose('iso', …)` against an offscreen 512×512 container (square, because the landing card's figure is a 64px square with `object-fit: cover` — a landscape capture would crop the vessel's ends away), so lighting, background and framing come from the viewer's own code rather than a parallel set of constants. One session (one WebGL context) is reused across a whole publish; photo overlay textures are deliberately not decoded for a 64px tile.
- **Best effort, never blocking.** No WebGL, a blocked readback, or a throw mid-build yields no screenshot, one `console.warn`, and a publish that carries on. The client page falls back to its typographic card — the same path an older bundle takes. Upload/flip ordering is untouched.
