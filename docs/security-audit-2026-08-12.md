...
# Remediation status (2026-08-12/13)

Implemented on branch `worktree-security-audit-remediation` (worktree off master `ce47482`). 8 opus implementation agents + 2 independent adversarial reviewers (both initially returned **do-not-merge** with confirmed defects) + 2 fixup agents. Gate: `npm run build` ✓ (2m1s), `vitest run --coverage` exit 0 (65.1% statements ≥ 50% threshold), `npm run lint` 0 errors, `tsc --noEmit` clean.

## Fixed in this branch

| Finding | Status | Where |
|---|---|---|
| C1 | **Fixed** — `gemini-proxy` is a true relay: key only in server-side `x-goog-api-key` header, model allowlist regex, Gemini body forwarded verbatim (HTTP 200 even on model errors, preserving the client `errorStatus` contract); client key-fetch deleted; `.env.example` VITE key line removed | `supabase/functions/gemini-proxy/index.ts`, `src/components/VesselModeler/services/gemini-proxy.ts` |
| C2 | **Fixed** — `manager` org-scoped on profiles / employee_competencies / competency_documents SELECT **and** on the competency-docs *storage* policy (reviewer catch); `approve_permission_request` + `reject_permission_request` NULL-role-safe, elevated-role allowlist (manager/admin/super_admin need admin+ caller), `EXECUTE` revoked from `PUBLIC`/`anon` (reviewer-found anon self-promotion chain closed) | migrations `20260812120000`, `20260812121000` |
| H1 | **Fixed** — controlled-docs storage SELECT scoped via `SECURITY DEFINER controlled_document_org()` = caller org (definer avoids the nested-RLS status-gate regression, reviewer catch); anti-OR-widening sweep incl. bucket-wide-policy warning arm | migration `20260812121000` |
| H2 | **Fixed** — `vessel-annotations` private, image-only MIME, 10MB cap, org-folder-scoped INSERT/UPDATE/DELETE (+`super_admin` escape), authenticated SELECT; client uploads org-prefixed paths, validates type/size, serves 1h signed URLs, tolerates legacy stored public URLs. Accepted residual: org-wide (not per-org) read; legacy pre-org objects immutable except via service role | migration `20260812124000`, `src/services/annotation-attachment-service.ts` |
| H3 | **Fixed** — labels built with DOM APIs/`textContent` (no interpolated `innerHTML`); image URLs must pass `isSafeImageUrl` (`https:`/`data:image/` only) on both sync and async `img.src` paths. Adversarial reviewer constructed no bypass | `src/components/VesselModeler/engine/annotation-labels.ts` |
| H4 | **Fixed** — both deletion functions remove Storage (`competency-documents/<uid>/`, avatars via path + `avatar_url`, `document_url` strays) before auth-user deletion | `supabase/functions/delete-user`, `delete-my-account` |
| H5 | **Fixed** — `org_admin` org-scoped on competency history SELECT + comment UPDATE/DELETE | migration `20260812120000` |
| H6 | **Mostly fixed** — `npm audit fix`: 9 High + 6 Moderate → **1 High + 4 Moderate**; remainder (`nanoid` H; `exceljs`, `react-router`, `react-router-dom`, `uuid` M) needs semver-major bumps — user decision | `package-lock.json` |
| M1 | **Fixed** — `vercel.json` ships CSP (strict `script-src 'self'`; `style-src` +`'unsafe-inline'` deliberately — ~2.2k React inline styles; **no** `upgrade-insecure-requests` — would risk the Companion localhost ports; Gemini origin removed here and from both `vite.config.js` header blocks), HSTS 2y `includeSubDomains; preload`, XFO/XCTO/Referrer/Permissions | `vercel.json`, `vite.config.js` |
| M3 | **Fixed** — shared `_shared/role-rank.ts`: strictly-lower-rank rule in `delete-user` (+ sole-admin/self guards kept), `admin-update-email`, `create-user`, `bulk-create-users`, **and `approve-account-request`** (reviewer catch — it could still mint an admin); admin→admin minting now super_admin-only | edge functions |
| M4 | **Fixed** — `sync-users` always recreates profiles as `viewer` | `supabase/functions/sync-users` |
| M5 | **Fixed** — `send-email`: recipients must resolve (case-insensitively) to profiles in the caller's org (admin+: any profile), header passthrough allowlisted to the one used set, ≤50 recipients / ≤200KB, all logs email-masked; `super_admin` admitted to `requireOrgAdmin` (was 403'd) | `supabase/functions/send-email`, `_shared/auth.ts` |
| M6 | **Fixed** — erasure deletes `competency_documents` rows + files and anonymises the user linkage on org `documents` (business records not deleted — deliberate); export now includes both tables (superset of erasure) | deletion fns, `src/services/gdpr-service.ts` |
| M7 | **Fixed** — per-step failure aggregation; any real failure → 500 with step list and the auth user is **not** deleted (retryable); missing-relation errors (PGRST202/204/205, 42P01/42703) skip-with-warning (reviewer blocker: phantom `notification_emails` table / `personnel.witnessed_by` column would have bricked every deletion — corrected to `notification_email_log` / `notification_email_recipients` / `employee_competencies.witnessed_by`); admin client now surfaces the real 409/500 bodies | deletion fns, `src/auth/auth-users.ts` |
| M9 | **Fixed** — `send-reset-code` returns one generic 200 in all cases (exists / missing / rate-limited), bookkeeping runs regardless of account existence | `supabase/functions/send-reset-code` |
| M11 | **App side fixed (2026-08-27); bucket flip PARKED, not applied** — `vessel-images` private; `avatars` MIME narrowed to images (PDF out). Steps 1-2 of the migration comment's 4-step plan are done: uploads persist the object PATH (`useUploadAvatar`), reads resolve through one tolerant both-shapes helper (`services/avatar-service.ts` — legacy public URL passes through, path → 1h signed URL, batched via `hooks/queries/useAvatarUrls.ts`), and both render surfaces degrade to initials on load failure. Steps 3-4 (backfill URL→path, drop the `TO public` avatars SELECT, `public=false`) are written and **held** in `database/parked-migrations/avatars_private.sql` — apply WITH or AFTER the frontend deploy, never before | migration `20260812122000`, `src/services/avatar-service.ts`, `database/parked-migrations/avatars_private.sql` |
| L2 | **Fixed** — user logout `scope:'global'` | `src/auth/auth-supabase.ts` |
| L3 | **Partial (client)** — both reset flows route through `validatePasswordStrength` gated on `feedback.length===0` (legacy modal floor 6→12 chars, email-in-password check active); client special-char set aligned to the server's exact set. GoTrue server limits still to verify | `LoginPageNew.tsx`, `auth-password-reset-form.ts`, `config/security.ts` |
| L4 | **Fixed** — `protect_sensitive_profile_fields` now BEFORE INSERT OR UPDATE (INSERT: service-role contexts pass, non-admins coerced to `viewer` + `RAISE LOG`); legacy `"Allow profile creation"` + tautological `"Admins can create users"` INSERT policies replaced by one helper-based policy (unaliased-subquery cross-org insert hole closed — reviewer catch) | migration `20260812123000` |
| L5 | **Fixed** — loud DEPRECATED/DO-NOT-RUN banners on both restore scripts (regressions enumerated; files kept for forensics) | `database/restore-rls-from-csv-export.sql`, `database/pii-lockdown-restore.sql` |
| L6 | **Fixed** — constant-time compare (no length short-circuit) for cron secret + reset code | `_shared/auth.ts`, `verify-reset-code` |
| L7 | **Fixed** — `submit-account-request` validates `organization_id` exists (client contract always supplies it — verified) + duplicate-request dedupe-as-throttle; `approve-account-request` refuses (409) when the email already has an account | edge functions |
| L8 | **Fixed** — client validation on controlled-doc uploads (UI-accept mirror + 25MB) and avatars (png/jpeg/webp ≤5MB; GIF dropped); recipient emails masked in send-email logs | services/hooks, `send-email` |

## Not addressed (deliberate — needs user decision or live access)

- **M2** AAL2 at the data layer — security-architecture decision (which tables/ops require 2FA), not taken unilaterally.
- **M8** retention cron scheduling — live pg_cron op.
- **M10** self-service competency model — product decision (verified-gate vs remove self-write).
- **M12** — sweep improved, but proof = dump/diff live `pg_policies` (live op).
- **L1** localStorage sessions — accepted for now *with* the new CSP; cookie-session migration is a separate project.
- **Audit gap** — `admin-reset-2fa` + `manage-backup-codes` still absent from the repo.

## New issues surfaced during remediation (pre-existing, not fixed here)

1. **`approve_permission_request` probably already broken for `org_admin` in production**: its `UPDATE profiles SET role` fires `protect_sensitive_profile_fields` with the caller's `auth.uid()`, whose UPDATE branch restricts role changes to admin/super_admin. Needs a decision (definer-context bypass flag or admin-only approvals). **RESOLVED 2026-08-27:** defect CONFIRMED by container reproduction, fixed by migration `20260827120000_profiles_role_guard_definer_context.sql` (adversarially reviewed, applied to production; guard converted to SECURITY INVOKER with owner-comparison vetting — a session-GUC flag was rejected as client-settable; the fix also closed a pre-existing NULL-role hole letting a no-profile-row JWT caller mint super_admin through definer functions). Consequence accepted pending owner review: org_admin can now approve in-org `org_admin` requests — this is `20260812120000`'s pre-existing elevated-role allowlist, previously masked by the defect; tightening it means editing that allowlist. Invariant recorded in the Decision Log: definer functions updating role/organization_id carry their own authorization, and the two weaker hand-run copies of this RPC were neutralized with DO-NOT-RE-ADD blocks.
2. **Orphaned-admin trap**: `sync-users` recreates a missing admin profile as `viewer` and `admin-update-email` 404s until sync — an admin whose profile row vanishes can't be restored via edge functions.
3. **`avatars` may hold legacy PDFs** (world-readable while public) — one-off inventory needed.
4. Deleting a user who owns controlled documents now 409s with a reassign-first message (new, intentional behavior — was silent orphaning).

## Live-project actions (user checklist)

1. **ROTATE `GEMINI_API_KEY` now** (assume compromised — it was vended to every authenticated user and travelled in Gemini URL query strings). Set the new secret, then deploy edge functions: `gemini-proxy`, `delete-user`, `delete-my-account`, `admin-update-email`, `create-user`, `bulk-create-users`, `sync-users`, `send-email`, `send-reset-code`, `verify-reset-code`, `submit-account-request`, `approve-account-request` (shared `_shared/*` changes ride along).
2. **Confirm the `manager` decision**: this branch treats `manager` as customer-facing and org-scopes it everywhere (consistent with the 2026-06 activity-log de-scoping). A `manager` with `organization_id IS NULL` now sees **nobody** — verify no active account is in that state (esp. Matrix-internal staff).
3. **Before `db push`**: dump live `pg_policies` for `profiles` and `storage.objects` — confirm which profiles-SELECT shape production runs (the migration installs the helper-based form; if live was a looser any-same-org-read shape, plain editor/viewer visibility narrows to self) and that `20260629120000` (competency-docs folder scoping) is applied live.
4. **Deploy ordering**: migration `20260812124000` (annotations bucket → private) and the frontend must ship **together** — the migration alone breaks existing annotation images (stored public URLs stop resolving).
5. Dashboard checks (unchanged from the audit): `avatars`/`vessel-images` public flags, `documents` bucket MIME/size limits, anonymous sign-in off, GoTrue rate limits + min-password policy, pg_cron retention jobs.
6. **HSTS `preload` is a one-way commitment** — confirm every `matrixportal.io` subdomain is HTTPS-only before submitting to hstspreload.org; after deploy, `curl -I` the site and watch the console for CSP violations (no `report-uri` is configured, so breakage is silent).
7. Remaining npm advisories need major bumps (`npm audit fix --force` territory) — decide per package.
8. Pull `admin-reset-2fa` + `manage-backup-codes` into the repo and audit them.
9. Post-deploy smoke tests: login as all 6 roles; personnel list as `manager`; competency doc review as `super_admin`/`manager`; controlled-doc download as `viewer` (incl. a superseded parent doc); annotation image upload + reload; GA drawing import (3-sample ensemble through the new proxy); delete a test user end-to-end; GDPR export (print via Ctrl+P — the inline print button was removed for CSP).
