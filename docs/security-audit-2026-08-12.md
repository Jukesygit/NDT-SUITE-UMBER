...
| M11 | **Partial** — `vessel-images` private (zero `src/` references verified); `avatars` MIME narrowed to images (PDF out). `avatars` public flag **deferred**: the app serves avatars via permanent `getPublicUrl` persisted in `profiles.avatar_url` — flipping now breaks every avatar; 4-step signed-URL refactor plan in the migration comment | migration `20260812122000` |
