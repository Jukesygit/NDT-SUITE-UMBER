-- ================================================
-- MASTER RLS FIX MIGRATION SCRIPT V2
-- ================================================
...
--
-- V2: Uses public schema instead of auth schema (Supabase compatible)
-- ================================================
...
CREATE POLICY "profiles_update_policy"
    ...
    WITH CHECK (
        CASE
            WHEN id = auth.uid() AND NOT public.auth_is_admin() THEN
                role = OLD.role AND
...
CREATE POLICY "employee_competencies_update_policy"
    ...
    WITH CHECK (
        CASE
            WHEN user_id = auth.uid() AND NOT public.auth_is_admin() AND NOT public.auth_is_org_admin() THEN
                status != 'approved'
            ELSE true
        END
    );
