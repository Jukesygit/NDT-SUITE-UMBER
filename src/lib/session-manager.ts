...
// Session LENGTH is a separate concern and lives in ./session-timebox.ts:
// this module keeps a session alive by refreshing tokens; the server's
// time-box decides when it stops being allowed to.

// Configuration constants (token-refresh coordination only)
...
class SessionManager {
  ...
  private async executeRefresh(): Promise<RefreshResult> {
    ...
    console.log(
      `[AUTH-DEBUG] SessionManager: refresh failed (attempt ${this.consecutiveFailures}), NOT forcing logout - waiting for next cycle`
    );
  ...
  private startProactiveRefresh(): void {
    ...
    this.refreshInterval = setInterval(async () => {
      ...
      if (this.hadRecentAuthEvent()) {
        console.log(
          '[AUTH-DEBUG] SessionManager: skipping proactive refresh - recent auth event detected'
        );
        return;
      }
      ...
      console.log(
        `[AUTH-DEBUG] SessionManager: proactive refresh result: ${result.reason || 'unknown'}`
      );
