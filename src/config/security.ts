...
export function validatePasswordStrength(
  ...
  // SECURITY: this set must be IDENTICAL to the server's
  // (supabase/functions/_shared/password-validation.ts → specialChars:
  // '!@#$%^&*()_+-=[]{}|;:,.<>?'). The client previously also accepted
  // ' " \ and /, so a password whose only "special" character was one of
  // those passed the client form and was then rejected by the server.
  ...
  if (PASSWORD_POLICY.preventUserInfo && userInfo) {
    ...
    const containsUserInfo = userFields.some(
      (field) => field && (lowerPassword.includes(field) || field.includes(lowerPassword))
    );
  ...
  if (PASSWORD_POLICY.maxConsecutiveChars) {
    ...
    if (!hasExcessiveConsecutive) {
      ...
      result.feedback.push(
        `Password should not contain more than ${PASSWORD_POLICY.maxConsecutiveChars - 1} consecutive identical characters`
      );
      result.score = Math.max(0, result.score - 1);
    }
  }
  ...
  result.isValid =
    result.score >= PASSWORD_POLICY.minStrengthScore &&
