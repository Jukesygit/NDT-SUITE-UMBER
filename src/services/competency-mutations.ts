...
export async function deleteCompetency(competencyId: string): Promise<boolean> {
  ...
  const { error } = await sb.from('employee_competencies').delete().eq('id', competencyId);
  if (error) throw error;
...
export async function setCompetencyDocuments(
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    if (existingId) {
      ...
      const { error: insertError } = await sb.from('competency_documents').insert({
...
export async function getDocumentUrls(paths: string[]): Promise<Record<string, string>> {
  ...
  const { data, error } = await sb.storage.from('documents').createSignedUrls(toSign, 3600);
  if (error) throw error;
...
export async function bulkCreateCompetencies(competencies: any[]): Promise<any[]> {
  ...
  const { data, error } = await sb
    .from('employee_competencies')
    .upsert(competencies, { onConflict: 'user_id,competency_id' })
    .select();
  if (error) throw error;
  return data;
}
...
export async function bulkImportCompetencies(
  ...
  for (const [fieldName, value] of Object.entries(csvData)) {
    ...
    const definition = definitions.find(
      (def: any) => def.name.toLowerCase() === fieldName.toLowerCase()
    );
    if (definition) {
  ...
  const { data, error } = await sb
    .from('employee_competencies')
    .upsert(competenciesToImport, { onConflict: 'user_id,competency_id' })
    .select();
  if (error) throw error;
  return data;
}
