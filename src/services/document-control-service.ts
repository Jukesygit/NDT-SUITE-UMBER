...
export async function getDocuments(filters?: DocumentFilters): Promise<Document[]> {
  ...
  if (filters?.search) {
    ...
    if (term) {
      query = query.or(
        `title.ilike.%${term}%,doc_number.ilike.%${term}%,description.ilike.%${term}%`
      );
    }
  }
...
export async function getDocument(
  documentId: string
): Promise<Document & { current_revision?: DocumentRevision }> {
  if (!isSupabaseConfigured()) throw new Error('Supabase not configured');
...
export async function submitForReview(revisionId: string): Promise<DocumentRevision> {
  ...
  if (current.status !== 'draft') {
    throw new Error(
      `Cannot submit for review: revision is currently "${current.status}" (must be "draft")`
    );
  }
  ...
  await supabase.from('documents').update({ status: 'under_review' }).eq('id', data.document_id);
...
export async function approveRevision(
  ...
  if (current.status !== 'under_review') {
    throw new Error(
      `Cannot approve: revision is currently "${current.status}" (must be "under_review")`
    );
  }
  ...
  if (doc) {
    ...
    await supabase.from('document_review_schedule').insert({
...
export async function rejectRevision(
  ...
  if (current.status !== 'under_review') {
    throw new Error(
      `Cannot reject: revision is currently "${current.status}" (must be "under_review")`
    );
  }
...
// Mirrors the `accept` attribute of the document upload inputs
// (CreateDocumentModal / CreateRevisionModal). Both extension and MIME type are
// checked so a renamed file cannot slip past either gate.
const ALLOWED_DOCUMENT_EXTENSIONS = ['pdf', 'jpg', 'jpeg', 'png', 'gif', 'webp'];
...
const MAX_DOCUMENT_FILE_BYTES = 25 * 1024 * 1024; // 25MB
...
export async function uploadDocumentFile(
  ...
  if (!fileExt || !ALLOWED_DOCUMENT_EXTENSIONS.includes(fileExt)) {
    throw new Error(
      `Invalid file type. Allowed types: ${ALLOWED_DOCUMENT_EXTENSIONS.map((ext) => `.${ext}`).join(', ')}`
    );
  }
  ...
  if (file.size > MAX_DOCUMENT_FILE_BYTES) {
    throw new Error(
      `File is too large. Maximum size is ${MAX_DOCUMENT_FILE_BYTES / (1024 * 1024)}MB.`
    );
  }
...
export async function getDocumentsDueForReview(daysAhead = 30): Promise<ReviewDueDocument[]> {
  ...
  const { data, error } = await supabase.rpc('get_documents_due_for_review', {
...
export async function completeReviewNoChanges(
  documentId: string,
  reviewNotes?: string
): Promise<void> {
  if (!isSupabaseConfigured()) throw new Error('Supabase not configured');
