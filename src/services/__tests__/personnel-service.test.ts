...
const { mockChain, mockIsConfigured, mockGetCurrentUser } = vi.hoisted(() => {
  function buildChain(resolved = { data: [], error: null }) {
    ...
    chain.storage = {
      from: vi.fn(() => ({ upload: vi.fn(), remove: vi.fn(), createSignedUrl: vi.fn() })),
    };
...
describe('getAllPersonnelWithCompetencies', () => {
  it('combines profiles with grouped competencies', async () => {
    const payloadsByTable: Record<string, any> = {
      profiles: {
        ...
        data: [
          {
            ...
            competency_definitions: {
              id: 'd1',
              name: 'UT',
              competency_categories: { id: 'c1', name: 'NDT' },
            },
          },
        ],
        error: null,
    ...
    // Each `.from(table)` call gets its own chain object whose `.then` is bound
    // to that table via closure. This resolves correctly regardless of which
    // query's `.then` microtask actually fires first — unlike a shared counter
    // or a single mutable "last table" field, both of which are order-dependent
    // and break when the competencies query's fallback wrapper changes the
    // relative timing of the two queries' resolutions.
    mockChain.from.mockImplementation((table: string) => {
      const tableChain: Record<string, any> = {};
      const self = () => tableChain;
      for (const m of [
        'select',
        'insert',
        'update',
        ...
        tableChain[m] = vi.fn(self);
      }
      tableChain.then = (resolve: any, reject?: any) =>
        Promise.resolve(payloadsByTable[table] ?? { data: [], error: null }).then(resolve, reject);
      return tableChain;
    });
  ...
  it('throws when supabase not configured', async () => {
    ...
    await expect(personnelService.getAllPersonnelWithCompetencies()).rejects.toThrow(
...
describe('exportPersonnelToCSV', () => {
  ...
  it('throws when no personnel data', async () => {
    await expect(personnelService.exportPersonnelToCSV([])).rejects.toThrow(
      'No personnel data to export'
    );
  });

  it('throws when personnel is null', async () => {
    await expect(personnelService.exportPersonnelToCSV(null as any)).rejects.toThrow(
      'No personnel data to export'
    );
  });
});
...
describe('getOrganizationPersonnelStats', () => {
  ...
  it('throws when supabase not configured', async () => {
    mockIsConfigured.mockReturnValueOnce(false);
    await expect(personnelService.getOrganizationPersonnelStats('org-1')).rejects.toThrow(
      'Supabase not configured'
    );
  });
});
...
describe('bulkUpdateCompetencyStatus', () => {
  it('throws when user is not admin', async () => {
    ...
    await expect(personnelService.bulkUpdateCompetencyStatus([])).rejects.toThrow(
