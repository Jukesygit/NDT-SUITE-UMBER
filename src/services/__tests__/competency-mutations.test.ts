...
vi.mock('../competency-queries.ts', () => ({
  getCompetencyDefinitions: vi
    .fn()
    .mockResolvedValue([{ id: 'def-1', name: 'UT Level 2', field_type: 'text' }]),
}));
...
describe('upsertCompetency', () => {
  ...
  it('uses active status when no document URL is provided', async () => {
    mockChain.single.mockResolvedValueOnce({
      data: { id: 'ec1', created_at: 't', updated_at: 't', competency: { name: 'X' } },
      error: null,
    });
...
describe('deleteCompetency', () => {
  ...
  it('skips http and empty document values when collecting storage paths', async () => {
    mockChain.maybeSingle.mockResolvedValueOnce({
      data: { document_url: 'https://external/cert.pdf' },
      error: null,
    });
...
describe('verifyCompetency', () => {
  it('approves a competency', async () => {
    mockChain.single.mockResolvedValueOnce({
      data: { id: 'ec1', status: 'active', competency: { name: 'MT' } },
      error: null,
    });
  ...
  it('rejects a competency with reason', async () => {
    mockChain.single.mockResolvedValueOnce({
      data: { id: 'ec1', status: 'rejected', competency: { name: 'MT' } },
      error: null,
    });
...
describe('requestChanges', () => {
  it('updates status to changes_requested', async () => {
    mockChain.single.mockResolvedValueOnce({
      data: { id: 'ec1', status: 'changes_requested', competency: { name: 'PT' } },
      error: null,
    });
...
describe('bulkCreateCompetencies', () => {
  ...
  it('throws when argument is not an array', async () => {
    await expect(bulkCreateCompetencies(null as any)).rejects.toThrow(
      'Competencies array is required'
    );
  });
});
...
describe('bulkImportCompetencies', () => {
  ...
  it('throws when no matching competencies found', async () => {
    await expect(bulkImportCompetencies('u1', { NonExistent: 'value' })).rejects.toThrow(
