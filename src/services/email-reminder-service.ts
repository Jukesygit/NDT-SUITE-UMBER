...
import { DEFAULT_SENDER_EMAIL } from '../config/contact';
...
export async function getEmailReminderSettings(): Promise<EmailReminderSettings> {
  const { data, error } = await supabase!.from('email_reminder_settings').select('*').single();
...
export async function updateEmailReminderSettings(
  ...
  if (existingData?.id) {
    ...
    // Insert new settings
    result = await supabase!.from('email_reminder_settings').insert(updateData).select().single();
  }
 ...
 */
export async function getEmailReminderLogs(options?: {
  userId?: string;
  limit?: number;
...
export async function sendTestReminder(
  userId: string,
  email: string
): Promise<SendTestReminderResult> {
  // Get user's expiring competencies
  const { data: competencies, error: compError } = await supabase!.rpc(
  ...
  const userCompetencies = (competencies || []).filter(
