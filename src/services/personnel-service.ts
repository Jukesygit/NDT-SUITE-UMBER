...
class PersonnelService {
    ...
    allCompetencies.forEach((comp: Record<string, unknown>) => {
      ...
      competenciesByUser[userId].push({
        ...
        competency: {
          ...compDefs,
          category:
            (compDefs as Record<string, unknown> | undefined)?.competency_categories || null,
        },
      });
    });
  ...
  async exportPersonnelToCSV(personnel: Record<string, unknown>[]): Promise<string> {
    ...
    // Build CSV header
    const header = ['Name', 'Email', 'Organization', 'Role', ...sortedCompNames];
  ...
  async getOrganizationPersonnelStats(organizationId: string) {
    ...
    data.forEach((person: Record<string, unknown>) => {
      ...
      competencies.forEach((comp) => {
        ...
        if (
          comp.status === 'expired' ||
          (comp.expiry_date && new Date(comp.expiry_date as string) < new Date())
        ) {
          stats.expiredCompetencies++;
        } else if (comp.expiry_date) {
          const daysUntilExpiry = Math.ceil(
            (new Date(comp.expiry_date as string).getTime() - new Date().getTime()) /
              (1000 * 60 * 60 * 24)
          );
  ...
  async getPersonnelComplianceReport(userId: string) {
    ...
    (person as Record<string, unknown>).competencies = (competencies || []).map(
    ...
    const personCompetencies =
      ((person as Record<string, unknown>).competencies as Record<string, unknown>[]) || [];
    ...
    return {
      ...
      summary: {
        ...
        expiredCompetencies: personCompetencies.filter(
          (c) =>
            c.status === 'expired' ||
            (c.expiry_date && new Date(c.expiry_date as string) < new Date())
        ).length,
   ...
   */
  async bulkUpdateCompetencyStatus(
    updates: Array<{ userId: string; competencyId: string; status: string; notes?: string }>
  ) {
    if (!isSupabaseConfigured()) {
      throw new Error('Supabase not configured');
    }
    ...
    const results: {
      success: typeof updates;
      failed: Array<(typeof updates)[number] & { error: string }>;
    } = {
      success: [],
      failed: [],
    };
  ...
  async getComplianceDashboard() {
    ...
    const dashboard = {
      ...
      expiredCompetencies: 0,
      pendingApproval: 0,
      byOrganization: {} as Record<
        string,
        { personnel: number; competencies: number; active: number; expired: number }
      >,
    ...
    personnel.forEach((person: Record<string, unknown>) => {
      ...
      dashboard.byOrganization[orgName].active += competencies.filter(
        (c) => c.status === 'active'
      ).length;
      dashboard.byOrganization[orgName].expired += competencies.filter(
        (c) =>
          c.status === 'expired' ||
          (c.expiry_date && new Date(c.expiry_date as string) < new Date())
      ).length;
