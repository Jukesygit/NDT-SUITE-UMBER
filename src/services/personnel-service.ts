...
// @ts-nocheck - Service uses JS imports (supabase-client, auth-manager) that lack TypeScript definitions
...
/**
 * Statistics for an organization's personnel
 */
...
/**
 * Compliance report for a person
 */
interface ComplianceReport {
    person: Profile & { competencies: EmployeeCompetency[] };
    competenciesByCategory: Record<string, EmployeeCompetency[]>;
    history: any[];
    summary: {
        totalCompetencies: number;
        activeCompetencies: number;
...
class PersonnelService {
     ...
     * PERFORMANCE: Uses optimized 2-query pattern (Phase 1 optimization)
     */
    async getAllPersonnelWithCompetencies(): Promise<PersonnelWithCompetencies[]> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }

        // PERFORMANCE FIX: Use 2 optimized queries instead of N+1
        ...
        // Query 2: Get all competencies for all users in one query with WHERE IN
        const { data: allCompetencies, error: compError } = await supabase
        ...
        const competenciesByUser: Record<string, any[]> = {};
        (allCompetencies || []).forEach((comp: any) => {
            ...
            competenciesByUser[comp.user_id].push({
                ...
                witness_checked: comp.witness_checked,
                witnessed_by: comp.witnessed_by,
                witnessed_at: comp.witnessed_at,
        ...
        // Combine profiles with their competencies
        const profilesWithCompetencies = profiles.map((profile: any) => ({
            ...profile,
            competencies: competenciesByUser[profile.id] || []
        })) as PersonnelWithCompetencies[];

        console.log('Fetched profiles count:', profilesWithCompetencies.length);
        console.log('Total competencies fetched:', allCompetencies?.length || 0);
     ...
     */
    async getCompetencyMatrix(): Promise<CompetencyMatrix> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }
     ...
     * @param personnel - Array of personnel objects with competencies
     */
    async exportPersonnelToCSV(personnel: PersonnelWithCompetencies[]): Promise<string> {
        ...
        personnel.forEach(person => {
            (person.competencies || []).forEach((comp: any) => {
        ...
        // Build CSV rows
        const rows = personnel.map((person: any) => {
            const row: string[] = [
                person.username || '',
                person.email || '',
            ...
            sortedCompNames.forEach(compName => {
                const comp = (person.competencies || []).find((c: any) => c.competency?.name === compName);
                if (comp) {
     ...
     */
    async getOrganizationPersonnelStats(organizationId: string): Promise<OrganizationStats> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }
     ...
     */
    async searchPersonnel(searchTerm: string): Promise<PersonnelWithCompetencies[]> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }
     ...
     */
    async getPersonnelByRole(role: UserRole): Promise<PersonnelWithCompetencies[]> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }
     ...
     */
    async getPersonnelComplianceReport(userId: string): Promise<ComplianceReport> {
        if (!isSupabaseConfigured()) {
            throw new Error('Supabase not configured');
        }
        ...
        person.competencies = (competencies || []).map((comp: any) => ({
            ...comp,
            competency: {
                ...comp.competency_definitions,
        ...
        const competenciesByCategory: Record<string, EmployeeCompetency[]> = {};
        (person.competencies || []).forEach((comp: any) => {
        ...
        return {
            ...
            summary: {
                ...
                activeCompetencies: (person.competencies || []).filter((c: any) => c.status === 'active').length,
                expiredCompetencies: (person.competencies || []).filter((c: any) =>
                    c.status === 'expired' || (c.expiry_date && new Date(c.expiry_date) < new Date())
                ).length,
                pendingApproval: (person.competencies || []).filter((c: any) => c.status === 'pending_approval').length,
                withDocuments: (person.competencies || []).filter((c: any) => c.document_url).length
            }
        };
    }
     ...
     * @param updates - Array of {userId, competencyId, status, notes}
     */
    async bulkUpdateCompetencyStatus(updates: Array<{
        userId: string;
        competencyId: string;
        status: CompetencyStatus;
        notes?: string;
        ...
        for (const update of updates) {
            try {
                ...
                results.failed.push({ ...update, error: errorMessage });
            }
        }
     ...
     * @param userId - User ID
     * @param competencyId - Competency definition ID
     * @param issuedDate - Issued date (ISO string or null)
     * @param expiryDate - Expiry date (ISO string or null)
     * @param issuingBody - Issuing organization
     * @param certificationId - Certification number
     * @param value - Competency value
     */
     ...
     * @param issuedDate - Issued date (ISO string or null)
     * @param expiryDate - Expiry date (ISO string or null)
     * @param issuingBody - Issuing body/organization (for certifications)
     * @param certificationId - Certification ID/number
     * @param value - General value field
     */
    ...
    async getComplianceDashboard(): Promise<ComplianceDashboard> {
        ...
        personnel.forEach(person => {
            ...
            dashboard.byOrganization[orgName].active += competencies.filter((c: any) => c.status === 'active').length;
            dashboard.byOrganization[orgName].expired += competencies.filter((c: any) =>
                c.status === 'expired' || (c.expiry_date && new Date(c.expiry_date) < new Date())
            ).length;
