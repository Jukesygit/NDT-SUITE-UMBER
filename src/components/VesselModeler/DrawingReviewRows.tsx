...
export interface Cell { text: string; confidence: string; flags: string[]; edited: boolean; }
...
export interface NozzleCells {
  name: Cell; mount: Cell; orientation: Cell; pos: Cell; offset: Cell;
  elevation: Cell; proj: Cell; angle: Cell; size: Cell;
}
...
export const isResolved = (c: Cell, kind: FieldKind): boolean =>
  kind === 'orientation' ? orientResolved(c)
    : kind === 'nozzleOrientation' ? nozzleOrientResolved(c)
      : kind === 'mount' ? mountResolved(c)
        : kind === 'text' ? nameResolved(c) : numResolved(c);
...
export const statusOf = (c: Cell, kind: FieldKind): CellStatus => cellStatus(c, isResolved(c, kind));

const COMPACT: CSSProperties = { padding: '5px 7px', fontSize: '0.8rem' };
const TINT: Record<CellStatus, string | undefined> = {
  ok: undefined, missing: 'var(--color-warning)', flagged: 'var(--color-danger)', edited: 'var(--color-success)',
};
...
export function CellField({ cell, kind, options, title, onChange }: {
  cell: Cell; kind: FieldKind; options?: { value: string; label: string }[]; title?: string;
  onChange: (t: string) => void;
}) {
  const resolved = isResolved(cell, kind);
  const status = cellStatus(cell, resolved);
  const flagTitle = status === 'flagged'
    ? (cell.flags.length ? cell.flags.map((f) => FLAG_LABELS[f] ?? f).join('; ') : 'Low agreement between extraction passes')
    : undefined;
  const style = cellStyle(status);
  if (options) {
    return (
      <select className="vm-select" value={cell.text} style={style} title={flagTitle ?? title} onChange={(e) => onChange(e.target.value)}>
  ...
  return (
    <input className="vm-input" type={kind === 'number' ? 'number' : 'text'} value={cell.text} style={style}
      title={flagTitle ?? title} placeholder={resolved ? undefined : 'not read from drawing'}
...
const RemoveBtn = ({ title, onClick }: { title: string; onClick: () => void }) => (
  <button className="vm-btn-icon" title={title} style={{ color: 'var(--color-danger)' }} onClick={onClick}><Trash2 size={13} /></button>
);

export function NozzleTable({ rows, onCell, onRemove }: {
  ...
  return (
    <table className="w-full border-collapse">
      <thead>
        <tr>
          <th className={TH} style={thStyle}>Tag</th>
          <th className={TH} style={thStyle}>Mount</th>
          <th className={TH} style={thStyle} title="Radial nozzle or side-facing horizontal (EL cluster)">Orient</th>
          <th className={TH} style={thStyle}>Pos / Off (mm)</th>
          <th className={TH} style={thStyle}>Proj (mm)</th>
          <th
            className={TH}
            style={thStyle}
          ...
          <th className={TH} style={thStyle} title="Elevation from vessel centerline (signed mm) — horizontal nozzles only">EL (mm)</th>
          <th className={TH} style={thStyle}>Size (mm)</th>
      ...
      <tbody>
        {rows.map(({ n, i }) => {
          ...
          return (
            <tr key={i}>
              <td className="p-0.5"><CellField cell={n.name} kind="text" onChange={(t) => onCell(i, 'name', t)} /></td>
              <td className="p-0.5"><CellField cell={n.mount} kind="mount" options={MOUNT_OPTIONS} onChange={(t) => onCell(i, 'mount', t)} /></td>
              <td className="p-0.5"><CellField cell={n.orientation} kind="nozzleOrientation" options={ORIENT_OPTIONS} onChange={(t) => onCell(i, 'orientation', t)} /></td>
              <td className="p-0.5"><CellField cell={head ? n.offset : n.pos} kind="number" title={posTitle} onChange={(t) => onCell(i, posKey, t)} /></td>
              <td className="p-0.5"><CellField cell={n.proj} kind="number" onChange={(t) => onCell(i, 'proj', t)} /></td>
              <td className="p-0.5"><CellField cell={n.angle} kind="number" title={horizontal ? 'Facing side (90 = right, 270 = left)' : undefined} onChange={(t) => onCell(i, 'angle', t)} /></td>
              <td className="p-0.5">
                {horizontal
                  ? <CellField cell={n.elevation} kind="number" title="EL from CL (signed mm)" onChange={(t) => onCell(i, 'elevation', t)} />
                  : <span style={{ color: 'var(--text-tertiary)', fontSize: '0.8rem' }} title="Elevation applies to horizontal nozzles only">—</span>}
              </td>
              <td className="p-0.5"><CellField cell={n.size} kind="number" onChange={(t) => onCell(i, 'size', t)} /></td>
              <td className="p-0.5 text-center"><RemoveBtn title="Remove nozzle" onClick={() => onRemove(i)} /></td>
            </tr>
          );
        })}
      </tbody>
...
export function RotateAllControl({ onRotate }: { onRotate: (a: RotateAction) => void }) {
  const Btn = ({ label, action, title }: { label: string; action: RotateAction; title: string }) => (
    <button className="vm-toggle-btn" style={{ flex: 'none', width: 'auto', padding: '3px 8px', fontSize: '0.7rem' }}
      title={title} onClick={() => onRotate(action)}>{label}</button>
  );
  return (
    <div className="flex items-center gap-1">
      ...
      <Btn label="Mirror" action="mirror" title="Mirror every nozzle angle (flip clockwise/counter-clockwise for an opposite-end view)" />
    </div>
  );
}

export function SaddleTable({ rows, onCell, onRemove }: {
  ...
  return (
    <table className="w-full border-collapse">
      <thead>
        <tr>
          <th className={TH} style={thStyle}>Saddle</th>
          <th className={TH} style={thStyle}>Position (mm)</th>
          <th className={TH} style={thStyle} />
        </tr>
      </thead>
      <tbody>
        {rows.map(({ c, i }) => (
          <tr key={i}>
            <td className="p-0.5 whitespace-nowrap" style={{ color: 'var(--text-tertiary)', fontSize: '0.75rem' }}>Saddle {i + 1}</td>
            <td className="p-0.5"><CellField cell={c} kind="number" onChange={(t) => onCell(i, t)} /></td>
            <td className="p-0.5 text-center"><RemoveBtn title="Remove saddle" onClick={() => onRemove(i)} /></td>
          </tr>
        ))}
      </tbody>
    </table>
