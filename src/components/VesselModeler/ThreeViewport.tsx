...
import {
  ...
  type LabelDragContext,
} from './engine/annotation-labels';
import {
  createAllInspectionImageLabels,
  type InspectionImageClickHandler,
...
const ThreeViewport = forwardRef<ThreeViewportHandle, ThreeViewportProps>(function ThreeViewport(
  ...
  const applyEntityVisibility = useCallback(() => {
    ...
    const applyByUserData = (
      ...
      for (const m of meshes) {
        ...
        if (cfg) {
          m.visible =
            isEffectivelyVisible(cfg, category, activeLayers) && isBodyVisible(cfg.bodyId);
        }
      }
    };
    ...
    const interaction = new InteractionManager(
      ...
      vesselStateRef.current,
      {
        ...
        onDomeGizmoDatumMoved: (compositeId, phiDeg, thetaDeg) =>
          callbacksRef.current.onDomeGizmoDatumMoved?.(compositeId, phiDeg, thetaDeg),
        onDomeGizmoDirectionToggle: (compositeId, field) =>
          callbacksRef.current.onDomeGizmoDirectionToggle?.(compositeId, field),
        ...
        onPipeSegmentSelected: (pipelineId, segmentIndex) =>
          callbacksRef.current.onPipeSegmentSelected?.(pipelineId, segmentIndex),
        onPipeConnectionPointClicked: (pipelineId) =>
          callbacksRef.current.onPipeConnectionPointClicked?.(pipelineId),
  ...
  const updateWeldLabelPositions = useCallback((cam: THREE.PerspectiveCamera) => {
    ...
    for (const { label, weld } of labels) {
      if (weld.bodyId !== undefined) {
        ...
        const circAngle = weld.type === 'circumferential' ? frame.toLocal(camLocal).angle : 0;
      ...
      const imgDragCtx: LabelDragContext | undefined = manager
        ? {
            ...
            onAnnotationLabelOffsetChanged: (id, offset) =>
              callbacksRef.current.onAnnotationLabelOffsetChanged?.(id, offset),
            onInspectionImageLabelOffsetChanged: (id, offset) =>
              callbacksRef.current.onInspectionImageLabelOffsetChanged?.(id, offset),
            onDragEnd: () => callbacksRef.current.onDragEnd?.(),
          }
        : undefined;
      const imgLabels = createAllInspectionImageLabels(
        state,
        selectedInspectionImageId,
        clickHandler,
        imgDragCtx
    ...
    // Add annotation leader lines + CSS2D labels
    {
      ...
      state.annotations.forEach((ann) => {
        ...
        if (effectiveMode === 'table') {
          ...
          const label = createAnnotationLabel(
            ann,
            state,
        ...
        if (tableAnnotations.length > 0) {
          ...
          for (const { key, label } of groupDefs) {
            ...
            for (const ann of items) {
              const scanMm = Math.round((ann.angle / 360) * Math.PI * state.id - origin.scanMm);
              ...
              if (ann.severityLevel) {
                ...
                dot.style.backgroundColor = { red: '#ff3333', yellow: '#ffaa00', green: '#33cc33' }[
                  ann.severityLevel
                ];
  ...
  const updateSelectionHighlights = useCallback(() => {
    ...
    weldMeshesRef.current.forEach((weldGroup) => {
      ...
      if (isSelected && idx !== undefined && idx >= 0 && idx < state.welds.length) {
        const glowGroup = createWeldGeometry(
          state.welds[idx],
          state,
          materials.weldGlow as unknown as THREE.MeshStandardMaterial
        );
    ...
    if (pipelineGroup && selectedPipelineId && selectedPipeSegmentIdx >= 0) {
      ...
      pipelineGroup.traverse((child) => {
        if (child instanceof THREE.Mesh && child.userData?.type === 'pipeSegment') {
          child.material =
            child.userData.segmentId === selectedSegId
              ? materials.nozzleHighlight
              : materials.pipeline;
        }
      });
      ...
      const previewOutline = createRectOutline(
        {
          ...
          name: 'Preview',
          type: 'scan',
          pos: previewCoverageRect.pos,
          angle: previewCoverageRect.angle,
          width: previewCoverageRect.width,
          height: previewCoverageRect.height,
          color: previewCoverageRect.color,
          lineWidth: 2,
          showLabel: false,
        },
        state,
  ...
  useEffect(() => {
    ...
    const appendagesById = new Map(
      vesselState.appendages.map((appendage) => [appendage.id, appendage])
    );
    groups.forEach((group) => {
      const appendage = appendagesById.get(group.userData?.bodyId as string);
      if (!appendage) return;
  ...
  useEffect(() => {
    ...
    if (manager) {
      ...
      manager.setCardinalDirectionsVisible(
        vesselState.visuals.showCardinalDirections,
        vesselState.visuals.cardinalRotation ?? 0
      );
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(to bottom, ${vesselState.visuals.backgroundColor || '#111111'}, #000000)`,
      }}
    />
  );
});
