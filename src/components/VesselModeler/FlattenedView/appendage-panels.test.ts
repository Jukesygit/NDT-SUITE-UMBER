...
describe('strip weld & lug placement', () => {
  ...
  it('keeps the strip axis PHYSICAL — X increases with pos (never mirrored)', () => {
    ...
    const near = stripToCanvasX(
      projectCircWeld(weld({ type: 'circumferential', pos: 100 }), APP_OD).x1,
      view
    );
    const far = stripToCanvasX(
      projectCircWeld(weld({ type: 'circumferential', pos: 900 }), APP_OD).x1,
      view
    );
    expect(far).toBeGreaterThan(near);
    // pos 0 sits at the panel's left content origin.
    expect(stripToCanvasX(0, view)).toBeCloseTo(view.paddingLeft + view.marginX + view.offsetX, 9);
  });
});
