 ...
 */
export function getAxialOrientation(composites: ScanCompositeConfig[]): AxialOrientation | null {
  const ref = composites.find((c) => c.orientationConfirmed && c.data.length > 0);
  if (!ref) return null;
  return {
...
export function axialToIndexMm(posMm: number, ori: AxialOrientation | null): number {
  if (!ori) return posMm;
  return ori.indexDirection === 'forward' ? posMm - ori.indexStartMm : ori.indexStartMm - posMm;
}
 ...
 */
export function circumDisplayMm(
  circumMm: number,
 ...
 */
export function axialContentMm(axialMm: number, vesselLength: number, reversed: boolean): number {
 ...
 */
export function wrapCircumCenters(cyMm: number, radiusMm: number, circumference: number): number[] {
  const centers = [cyMm];
