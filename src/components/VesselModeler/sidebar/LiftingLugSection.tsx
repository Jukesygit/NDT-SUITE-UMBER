...
export function LiftingLugSection({
  ...
  return (
    <SubSection
      title="Lifting Lugs"
      count={vesselState.liftingLugs.length}
      isOpen={isOpen}
      onToggle={onToggle}
    >
      {/* Style toggle */}
          ...
          <div
            ...
            onDragStart={(e) => {
              e.dataTransfer.setData(
                'application/x-lifting-lug',
                JSON.stringify({ ...s, style: lugStyle })
              );
      ...
      {vesselState.liftingLugs.map((l, i) => {
        ...
        const mountLen =
          i === selectedLugIndex && sel?.bodyId
            ? (vesselState.appendages.find((a) => a.id === sel.bodyId)?.length ??
              vesselState.length)
            : vesselState.length;
        return (
          <React.Fragment key={i}>
            ...
            >
              <div className="vm-list-item-info">
                <strong>{l.name}</strong> &mdash; {l.swl}{' '}
                {l.style === 'trunnion' ? 'Trunnion' : 'Pad Eye'} @ {Math.round(l.pos)}mm,{' '}
                {Math.round(l.angle)}&deg;
              </div>
              <button
                ...
                <div className="vm-form-row">
                  <div className="vm-control-group">
                    ...
                    <input
                      ...
                      max={mountLen}
                      onChange={(e) =>
                        onUpdateLug(selectedLugIndex, { pos: Number(e.target.value) })
                      }
                    />
                  </div>
                  <div className="vm-control-group">
                    ...
                    <input
                      ...
                      max={360}
                      onChange={(e) =>
                        onUpdateLug(selectedLugIndex, { angle: Number(e.target.value) })
                      }
                    />
                  </div>
                </div>
