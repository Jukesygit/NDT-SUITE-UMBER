...
export interface ScanCompositeSectionProps {
  ...
  cloudComposites:
    | Array<{ id: string; name: string; width: number; height: number; created_at: string }>
    | undefined;
...
export function ScanCompositeSection({
  ...
  onSelectScanComposite,
  onImportComposite,
  onUpdateScanComposite,
  onRemoveScanComposite,
  cloudComposites,
  ...
  return (
    <SubSection
      title="Scan Composites"
      count={vesselState.scanComposites.length}
      isOpen={isOpen}
      onToggle={onToggle}
    >
      <button
      ...
      {vesselState.scanComposites.map((sc) => {
        ...
        const widthMm =
          sc.xAxis.length > 0 ? Math.round(sc.xAxis[sc.xAxis.length - 1] - sc.xAxis[0]) : 0;
        const heightMm =
          sc.yAxis.length > 0 ? Math.round(sc.yAxis[sc.yAxis.length - 1] - sc.yAxis[0]) : 0;
        return (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (!isNaN(v))
                            onUpdateScanComposite(sc.id, {
                              indexStartMm: Math.max(0, Math.min(vesselState.length, v)),
                            });
                        }}
                        style={{ width: 64 }}
                      />
                      ...
                      <div className="vm-control-group">
                        ...
                        {sc.sourceFiles && sc.sourceFiles.length > 0 ? (
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              onUpdateScanComposite(sc.id, {
                                sourceNdeFile: e.target.value || undefined,
                              })
                            }
                    ...
                    >
                      <div className="vm-control-group">
                        ...
                        <select
                          ...
                          value={sc.colorScale}
                          onChange={(e) =>
                            onUpdateScanComposite(sc.id, { colorScale: e.target.value })
                          }
                        >
          ...
          <div className="vm-modal">
            ...
            <div className="vm-modal-body">
              <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', margin: '0 0 10px' }}>
                Select a composite to import. You can adjust placement after import using the 3D
                gizmo or sidebar controls.
              </p>
              {cloudCompositesLoading ? (
                <p
                  style={{
            ...
            <div className="vm-modal-footer">
              ...
              {importingId && (
                <button className="vm-btn vm-btn-primary" onClick={handleImport}>
                  Import
                </button>
              )}
            </div>
