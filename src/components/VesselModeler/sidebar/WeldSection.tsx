...
export function WeldSection({
  ...
  return (
    <SubSection title="Welds" count={vesselState.welds.length} isOpen={isOpen} onToggle={onToggle}>
      ...
      >
        <Plus size={14} /> Add {weldType === 'circumferential' ? 'Circumferential' : 'Longitudinal'}{' '}
        Weld
      </button>
          ...
          <React.Fragment key={i}>
            ...
            >
              <div className="vm-list-item-info">
                <strong>{w.name}</strong> &mdash; {w.type === 'circumferential' ? 'Circ' : 'Long'} @{' '}
                {Math.round(w.pos)}mm
                {w.type === 'longitudinal' && w.angle !== undefined
                  ? `, ${Math.round(w.angle)}\u00B0`
                  : ''}
              </div>
              <button
