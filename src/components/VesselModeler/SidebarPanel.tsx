...
type SidebarSectionId =
  ...
  | 'inspection'
  | 'piping';
type AttachmentSubId = 'appendages' | 'nozzles' | 'liftingLugs' | 'welds' | 'supports' | 'piping';
...
export default function SidebarPanel(props: SidebarPanelProps) {
  ...
  const [activeSection, setActiveSection] = useState<SidebarSectionId | null>(
    isPipeMode ? 'dimensions' : 'projectInfo'
  );
        ...
        {isPipeMode && (
          <>
            <Section
              ...
              icon={<Ruler size={14} style={{ marginRight: 6 }} />}
        ...
        {!isPipeMode && (
          <>
            ...
            <Section
              ...
              isOpen={activeSection === 'attachments'}
              onToggle={() => toggle('attachments')}
              count={
                vesselState.appendages.length +
            ...
            >
              <AppendageSection
                vesselState={vesselState}
                selectedAppendageIndex={props.selectedAppendageIndex}
                onAddAppendage={props.onAddAppendage}
                onUpdateAppendage={props.onUpdateAppendage}
                onRemoveAppendage={props.onRemoveAppendage}
                onSelectAppendage={props.onSelectAppendage}
                isOpen={activeAttachmentSub === 'appendages'}
                onToggle={() => toggleAttSub('appendages')}
              />
              <NozzleSection
                vesselState={vesselState}
                selectedNozzleIndex={props.selectedNozzleIndex}
            ...
            <Section
              ...
              isOpen={activeSection === 'scanOverlay'}
              onToggle={() => toggle('scanOverlay')}
              count={
            ...
            <Section
              title="Inspection"
              defaultOpen={false}
              icon={<ClipboardCheck size={14} style={{ marginRight: 6 }} />}
              isOpen={activeSection === 'inspection'}
        ...
        {isPipeMode && (
          <>
            ...
            <Section
              ...
              isOpen={activeSection === 'scanOverlay'}
              onToggle={() => toggle('scanOverlay')}
              count={vesselState.textures.length + vesselState.scanComposites.length}
            >
              <ImageOverlaySection
                vesselState={vesselState}
                selectedTextureId={props.selectedTextureId}
