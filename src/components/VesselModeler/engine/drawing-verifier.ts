...
import type { ExtractedValue, ExtractionReview, ReviewNozzle } from './drawing-extraction-voting';
 ...
 */
export function verifyExtraction(review: ExtractionReview, hasTable: boolean): ExtractionReview {
  ...
  for (const n of nozzles) {
    const isHead = n.mount.value === 'head-left' || n.mount.value === 'head-right';
    ...
    if (isHead && n.radialOffset.value !== null) {
      ...
      const overRadius = id.value !== null && n.radialOffset.value >= id.value / 2;
...
export function toExtractionResult(review: ExtractionReview): ExtractionResult {
  ...
  const nozzles = review.nozzles.map((n, i) => {
    ...
    const radialOffset = isHead ? need(`nozzles[${i}].radialOffset`, n.radialOffset) : undefined;
    ...
    const elevation = isHorizontal ? need(`nozzles[${i}].elevation`, n.elevation) : undefined;
    return {
 ...
 */
export function reviewToLenientResult(review: ExtractionReview): ExtractionResult {
