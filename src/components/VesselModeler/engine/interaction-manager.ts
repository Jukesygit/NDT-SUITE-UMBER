...
export class InteractionManager {
  ...
  private onPointerMove(event: PointerEvent): void {
    ...
    if ((this.isDrawing || this.isDrawingCoverage || this.isDrawingRuler) && this.isDown) {
      ...
      const hit =
        this.drawBodyId === undefined
          ? hits[0]
          : hits.find((h) => hitBodyId(h.object) === this.drawBodyId);
      if (!hit) return;
      ...
      if (this.isDrawingRuler) {
        ...
        this.callbacks.onCoverageRectPreview(
          centerPos,
          centerAngle,
          width,
          height,
    ...
    if (this.dragType === 'annotation') {
      ...
      this.callbacks.onAnnotationMoved(
        this.selectedAnnotationIdx,
        drag.pos,
        drag.angle,
        res.bodyId
      );
  ...
  private resolveDrapeDrag(
    ...
    const onHead = (lowHead && (posH < 0 || storedPos < 0)) || posH > L || storedPos > L;
    if (!onHead) {
