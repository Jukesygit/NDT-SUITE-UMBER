...
function votedNozzle(perSample: Array<Map<string, RawNozzle>>, key: string): ReviewNozzle {
  ...
  const isHead = votedMount.value === 'head-left' || votedMount.value === 'head-right';
  const votedOrientation = voteEnum(perSample.map((m) => m.get(key)?.nozzleOrientation ?? null));
...
function voteSaddles(samples: RawExtraction[]): Array<{ pos: ExtractedValue<number> }> {
