...
function makeVessel(over?: Partial<VesselState>): VesselState {
  return { ...DEFAULT_VESSEL_STATE, id: 2 * R, length: L, headRatio: 2, orientation: 'horizontal', ...over };
}
  ...
  for (const head of ['left', 'right'] as const) {
    for (const center of [
      ...
      it(`corner vertices agree head=${head} phi=${center.centerPhi} theta=${center.centerTheta}`, () => {
        ...
        const fwdFirst = tangentOffsetsToSurface(basis, R, D, L, head, -scanRange / 2, -indexRange / 2);
        ...
        const fwdLast = tangentOffsetsToSurface(basis, R, D, L, head, scanRange / 2, indexRange / 2);
