...
describe('vessel-history', () => {
  ...
  describe('labels', () => {
    ...
    it("keeps the group's FIRST label through coalescing", () => {
      let h = createEmptyHistory();
      h = recordCheckpoint(h, makeVessel(0), { key: 'drag:nozzle:2', label: 'Move nozzle 2', at: 1000 });
      h = recordCheckpoint(h, makeVessel(1), { key: 'drag:nozzle:2', label: 'Move nozzle 2', at: 1200 });
      h = recordCheckpoint(h, makeVessel(2), { key: 'drag:nozzle:2', label: 'Move nozzle 2', at: 1500 });
      expect(h.past).toHaveLength(1);
