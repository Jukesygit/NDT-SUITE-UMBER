...
describe('structuralHash — per-entity visible is cosmetic (C13)', () => {
  ...
  it('is unchanged when visible toggles on a nozzle', () => {
    expect(
      structuralHash({ ...populated, nozzles: [{ ...populated.nozzles[0], visible: false }] })
    ).toBe(baseHash);
    expect(
      structuralHash({ ...populated, nozzles: [{ ...populated.nozzles[0], visible: true }] })
    ).toBe(baseHash);
  });
  it('is unchanged when visible toggles on a lifting lug', () => {
    expect(
      structuralHash({
        ...populated,
        liftingLugs: [{ ...populated.liftingLugs[0], visible: false }],
      })
    ).toBe(baseHash);
  });
  it('is unchanged when visible toggles on a saddle', () => {
    expect(
      structuralHash({ ...populated, saddles: [{ ...populated.saddles[0], visible: false }] })
    ).toBe(baseHash);
  });
  it('is unchanged when visible toggles on a weld', () => {
    expect(
      structuralHash({ ...populated, welds: [{ ...populated.welds[0], visible: false }] })
    ).toBe(baseHash);
  });
  it('is unchanged when visible toggles on a ruler', () => {
    expect(
      structuralHash({ ...populated, rulers: [{ ...populated.rulers[0], visible: false }] })
    ).toBe(baseHash);
  });
  it('is unchanged when visible toggles on a coverage rect', () => {
    expect(
      structuralHash({
        ...populated,
        coverageRects: [{ ...populated.coverageRects[0], visible: false }],
      })
    ).toBe(baseHash);
  });
});
