  ...
  const toggleTextureVisible = useCallback(
    (id: number) => {
      ...
      updateVessel(
        (prev) => ({
          ...prev,
          textures: prev.textures.map((t) => (Number(t.id) === id ? { ...t, visible: show } : t)),
        }),
        {
          label: `${show ? 'Show' : 'Hide'} texture ${target.name || `Texture ${id}`}`,
          at: Date.now(),
        }
      );
    },
  ...
  const toggleCoverageRectVisible = useCallback(
    (id: number) => {
      ...
      updateVessel(
        (prev) => ({
          ...prev,
          coverageRects: prev.coverageRects.map((r) => (r.id === id ? { ...r, visible: show } : r)),
        }),
        {
          label: `${show ? 'Show' : 'Hide'} coverage ${target.name || `Coverage ${id}`}`,
          at: Date.now(),
        }
      );
    },
