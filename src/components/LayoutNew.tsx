...
function getStoredLogoId(): string {
  try {
    return localStorage.getItem('dev-logo-variant') || 'gradient-shift';
  } catch {
...
function LayoutNew() {
  ...
  const handleLogoChange = (id: string) => {
    setLogoId(id);
    try {
      localStorage.setItem('dev-logo-variant', id);
    } catch {
      /* storage unavailable */
    }
  };
  ...
  return (
    <div className="app">
      ...
      {isMaintenanceMode ? (
        <div
          className="px-5 py-2.5 text-center text-sm border-b"
          style={{
            ...
            >
              <div
                style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                  background: v.id.startsWith('glitch') ? '#111' : 'transparent',
                  borderRadius: 3,
                }}
              >
                <v.component size={56} />
              ...
              <span
                style={{
                  fontSize: 8,
                  color: v.id === logoId ? '#10b981' : '#6a6865',
                  textAlign: 'center',
                  lineHeight: 1.2,
                  fontWeight: v.id === logoId ? 600 : 400,
                }}
              >
                {v.label}
              </span>
      ...
      <input
        ...
        onChange={(e) => {
          if (/^#[0-9a-f]{0,6}$/i.test(e.target.value)) {
            ...
            if (e.target.value.length === 7) applyColor(e.target.value);
          }
        }}
        style={{
          width: 72,
