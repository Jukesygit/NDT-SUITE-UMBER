...
import { Plus, ChevronRight, Ship, Cuboid } from 'lucide-react';
...
import { VesselPlanningStrip } from './VesselPlanningStrip';
...
export function ProjectVesselList({
  projectId,
  vessels,
  compositeCountByVessel,
}: ProjectVesselListProps) {
      ...
      {vessels.length === 0 ? (
        <div className="pj-card">
          <div className="pj-empty">
            ...
            <div className="pj-empty-text">
              Add vessels to start setting up this inspection project.
            </div>
            <button
        ...
        <div className="pj-card" style={{ overflow: 'hidden' }}>
          {vessels.map((v) => {
            ...
            const coveragePct =
              v.coverage_target_pct && v.coverage_actual_pct
              ...
              >
                <div className="pj-dash-vessel-name">
                  <span className="pj-dash-vessel-title">
                    {v.vessel_tag ? `${v.vessel_tag} — ` : ''}
                    {v.vessel_name}
                  </span>
                  {v.vessel_type && <span className="pj-dash-vessel-type">{v.vessel_type}</span>}
                </div>
                ...
                {/* Planning status — same engine numbers as the vessel's Coverage section */}
                <VesselPlanningStrip projectVesselId={v.id} />

                <button
                  className="pj-vessel-action-btn ghost"
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate(planScopeUrl(projectId, v.id));
                  }}
                  title="Open this vessel's model in the modeler to set coverage targets"
                >
                  <Cuboid size={12} />
                  Plan scope
                </button>
                ...
                <ChevronRight
                  size={14}
                  style={{ color: 'var(--clean-text-quaternary)', flexShrink: 0 }}
                />
              </div>
            );
          })}
      ...
      {showAddModal && (
        <Modal isOpen={true} title="Add Vessel" onClose={() => setShowAddModal(false)}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
            <button onClick={() => setShowAddModal(false)} className="pj-btn secondary">
              Cancel
            </button>
            <button
              onClick={handleAdd}
