...
export const validateToken = (token: RangeToken, axis: AxisFacts): boolean => {
  ...
  return (
    axis.truncatedLocal && axis.max !== null && Math.abs(tokenSpan - axis.max) <= SPAN_TOLERANCE
  );
};
...
export const parseCandidates = (
  ...
  const labelledCandidate = (m: RegExpMatchArray | null, axis: AxisFacts): AxisCandidate | null => {
    if (!m) return null;
