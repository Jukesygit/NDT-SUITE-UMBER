      ...
      {usesHalving && (
        <div className="flex items-start gap-2 p-3 rounded-lg text-sm" style={{ backgroundColor: '#3a2f0f' }}>
          ...
          <p className="text-amber-200/90">
            Some positions were inferred by halving doubled metadata (Source:
            &quot;metadata ÷2&quot;) — a batch-level heuristic. Verify them with the
            overlap cross-check below before applying.
          </p>
        </div>
      )}
      ...
      {truncatedFiles.length > 0 && (
        <div className="flex items-start gap-2 p-3 rounded-lg text-sm" style={{ backgroundColor: '#3a2f0f' }}>
          ...
          <div className="text-amber-200/90">
            <p className="font-medium">Truncated export{truncatedFiles.length !== 1 ? 's' : ''} detected</p>
            {truncatedFiles.map(f => (
              <p key={f.filename} className="font-mono text-xs mt-1" title={f.filename}>
                {f.filename}: {f.actual} of {f.expected} rows — the missing region will
                stay empty. Re-export from the instrument if possible.
              </p>
            ))}
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg text-sm" style={{ backgroundColor: '#111827' }}>
          ...
          <div className="max-h-32 overflow-y-auto space-y-1">
            {checks.map(c => (
              ...
              >
                {c.verdict === 'confirmed' && <CheckCircle2 className="w-3.5 h-3.5 text-green-500 shrink-0" />}
                {c.verdict === 'mismatch' && <XCircle className="w-3.5 h-3.5 text-red-500 shrink-0" />}
                {c.verdict === 'insufficient' && <MinusCircle className="w-3.5 h-3.5 text-gray-600 shrink-0" />}
                <span className="truncate">
                  {c.fileA} ↔ {c.fileB}
                </span>
                <span className="ml-auto shrink-0 font-mono">
                  {c.verdict === 'insufficient' ? 'no overlap data' : `Δ ${c.meanAbsDiff.toFixed(3)} mm`}
                </span>
              </div>
            ))}
          </div>
          {mismatched.length > 0 && (
            <p className="mt-2 text-xs text-red-300/90">
              Disagreeing overlaps usually mean a tile is still misplaced — check the
              Source column and try &quot;Prioritize filenames&quot; before applying.
            </p>
          )}
        </div>
      )}
