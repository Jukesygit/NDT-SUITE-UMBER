...
function buildCABins(nwt: number, ca: number): BinResult[] {
  const t = [nwt, nwt - 0.33 * ca, nwt - 0.67 * ca, nwt - ca];
  return [
    {
      minMm: t[0],
      maxMm: Infinity,
  ...
  for (let ci = confirmed.length - 1; ci >= 0; ci--) {
    ...
    for (let row = 0; row < data.length - 1; row++) {
      for (let col = 0; col < data[row].length - 1; col++) {
        ...
        if (
          footprints.length > 0 &&
          footprints.some((fp) => fp.containsCell(cell.posMid, cell.angleMid))
        ) {
          continue;
        }
...
export function compute(req: WallLossRequest): WallLossResponse {
  ...
  const {
    composites,
    domeComposites,
    footprints: footprintParams,
    bodies: appendageBodies,
    vesselId,
    vesselLength,
    headRatio,
    nominalThickness,
    ...
    customBoundaries,
    corrosionAllowance,
    shellNominalThickness,
    domeNominalThickness,
  } = req;
