// @ts-nocheck - Large page component (2,599 lines) being migrated to TypeScript before splitting into smaller components
...
export default function PersonnelManagementPage() {
    const [view, setView] = useState('directory'); // directory, matrix, expiring, pending
    ...
    const loadData = async (showSuccess = false) => {
        ...
        try {
            setLoading(true);
            const [personnelData, orgsData, expiringData, competencyDefs, pendingData] = await Promise.all([
    ...
    const handleExportToCSV = async () => {
        try {
            ...
            toast.success(`Exported ${filteredPersonnel.length} personnel records successfully!`);
        } catch (error) {
...
function DirectoryView({ personnel, searchTerm, setSearchTerm, filterOrg, setFilterOrg, filterRole, setFilterRole, filterCompetencies, setFilterCompetencies, organizations, competencyDefinitions, onExport, onSelectPerson, getCompetencyStats, onImport, sortColumn, sortDirection, onSort, onRefresh }) {
...
function PendingApprovalsView({ pendingApprovals, onRefresh }) {
    const [approving, setApproving] = useState(null);

    const handleApprove = async (competencyId, approved, reason = null) => {
                    ...
                    {(!person.competencies || person.competencies.length === 0) ? (
                        <div style={{ padding: '16px 0' }}>
                            <EmptyData
                                ...
                                description="No competencies or certifications have been recorded for this person yet."
                            />
                        </div>
                    ) : (
                        <div className="space-y-3">
