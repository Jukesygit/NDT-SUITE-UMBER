...
import { isNeverHiddenTab } from '../../../services/tab-visibility-service';
import { SectionSpinner, ErrorDisplay } from '../../../components/ui';

export default function TabVisibilityTab() {
  const { data: allSettings = [], isLoading, isError } = useTabVisibility();
  const updateMutation = useUpdateTabVisibility();

  // The system tabs are omitted rather than rendered as locked rows: the list
  // is a short column of switches, and a dead switch invites the click that
  // this exclusion exists to prevent. `admin` is the tab hosting THIS screen —
  // hiding it would remove the only way to unhide it — and `profile` is the
  // route guard's redirect target. See NEVER_HIDDEN_TABS for the full reason.
  const settings = allSettings.filter((setting) => !isNeverHiddenTab(setting.tab_id));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold mb-1" style={{ color: 'var(--green-bright)' }}>
          Tab Visibility
        </h2>
        <p className="text-sm" style={{ color: 'rgba(53, 160, 88, 0.45)' }}>
          Control which navigation tabs are visible. Hiding a tab hides it for everyone, including
          super admins, and blocks its pages. Admin and Profile are system tabs and cannot be
          hidden.
        </p>
      </div>
        ...
        {settings.map((setting) => (
          ...
          >
            <div>
              ...
              <span
                style={{ marginLeft: '12px', fontSize: '12px', color: 'rgba(53, 160, 88, 0.30)' }}
              >
                ({setting.tab_id})
              </span>
            </div>
            ...
            <button
              ...
              style={{
                ...
                border: 'none',
                backgroundColor: setting.is_visible
                  ? 'var(--green-bright)'
                  : 'rgba(53, 160, 88, 0.20)',
