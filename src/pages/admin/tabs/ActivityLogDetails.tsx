...
export function DetailsView({ details }: { details: Record<string, unknown> | null }) {
  ...
  const changes = (
    details as {
      changes?: Record<
        string,
        { old?: unknown; new?: unknown; changed?: boolean; pii_redacted?: boolean }
      >;
    }
  ).changes;
      ...
      <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {Object.entries(changes).map(([field, change]) => {
            ...
            const bulkOnly =
              change?.changed && change.old === undefined && change.new === undefined;
            return (
              <tr key={field} style={{ borderTop: '1px solid rgba(53, 160, 88, 0.15)' }}>
                <td style={{ padding: '4px 12px 4px 0', color: 'var(--green)' }}>
                  {titleCase(field)}
                </td>
                {redacted || bulkOnly ? (
                  <td
