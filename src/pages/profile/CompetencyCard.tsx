...
function getApprovalStatus(status?: Competency['status']) {
  switch (status) {
    case 'pending_approval':
      return { label: 'Pending Approval', badgeClass: 'pending', show: true };
    case 'rejected':
      return { label: 'Rejected', badgeClass: 'rejected', show: true };
    case 'changes_requested':
      return { label: 'Changes Requested', badgeClass: 'changes-requested', show: true };
    case 'expired':
      return { label: 'Expired', badgeClass: 'expired', show: true };
    default:
      return { label: '', badgeClass: '', show: false };
  }
}
...
export function CompetencyCard({
  ...
  return (
    <div className={`pf-competency-card${compact ? ' compact' : ''}`}>
        ...
        <div className="pf-card-details">
          {competency.level && (
            <div>
              ...
              <span className="pf-detail-value" style={{ fontWeight: '600' }}>
                {competency.level}
              </span>
            </div>
          )}
      ...
      {approvalStatus.show ? (
        <div className={`pf-badge ${approvalStatus.badgeClass}`}>{approvalStatus.label}</div>
          ...
          <div style={{ minHeight: '400px' }}>
            ...
            {resolvedDocumentUrl && documentType === 'image' && (
              <div style={{ display: 'none', textAlign: 'center', padding: '40px' }}>
                <p className="pf-info-text" style={{ marginBottom: '12px' }}>
