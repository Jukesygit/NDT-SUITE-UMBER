...
import { ArrowLeft, Settings, Calendar, MapPin, Trash2, ChevronDown, Share2 } from 'lucide-react';
import {
  useProject,
  useProjectVessels,
  useProjectFiles,
...
import { ShareWithClientDialog } from '../../components/projects/ShareWithClientDialog';
...
export default function ProjectDetailPage() {
  ...
  for (const c of composites) {
    if (c.project_vessel_id) {
      compositeCountByVessel.set(
        c.project_vessel_id,
        (compositeCountByVessel.get(c.project_vessel_id) ?? 0) + 1
      );
    }
  }
      ...
      <div className="pj-header">
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="pj-page-meta">
            ...
            {vessels.length > 0 && (
              <span>
                {completedVessels}/{vessels.length} completed
              </span>
            )}
          </div>
        </div>
        ...
        <div className="pj-header-actions">
          ...
          <button
            ...
            title="Publish this project as a loginless client link"
      ...
      {showDeleteModal && (
        <Modal isOpen={true} title="Delete Project" onClose={() => setShowDeleteModal(false)}>
          <p style={{ marginBottom: 16 }}>
            Are you sure you want to delete <strong>{project.name}</strong>? This will remove all
            vessels, files, and data associated with this project. This action cannot be undone.
          </p>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={() => setShowDeleteModal(false)} className="pj-btn secondary">
              Cancel
            </button>
            <button
