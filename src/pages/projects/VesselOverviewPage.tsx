...
import { ArrowLeft, Box, Pencil, ChevronDown, ClipboardList, Eye, Cuboid } from 'lucide-react';
...
import { VesselCoverageScopeSection } from '../../components/projects/coverage-scope/VesselCoverageScopeSection';
import './projects.css';
...
export default function VesselOverviewPage() {
  ...
  const handlePopulateFromCompanion = useCallback(async () => {
    ...
    try {
      ...
      if (parts.length === 0) {
        setPopulateMsg({
          type: 'success',
          text: 'Nothing new to populate — all data already present.',
        });
      } else {
      ...
      setPopulateMsg({
        type: 'error',
        text: err instanceof Error ? err.message : 'Population failed',
      });
    }
  ...
  const commitTag = useCallback(() => {
    ...
    if (trimmed !== (vessel?.vessel_tag ?? '')) {
      updateVessel.mutate({
        id: vesselId!,
        params: { vesselTag: trimmed || undefined },
        projectId: projectId!,
      });
    }
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (statusMenuRef.current && !statusMenuRef.current.contains(e.target as Node))
        setStatusMenuOpen(false);
      if (modelerMenuRef.current && !modelerMenuRef.current.contains(e.target as Node))
        setModelerMenuOpen(false);
    };
    ...
    <div className="pj-page">
      ...
      <div className="pj-header" style={{ marginBottom: 0 }}>
        <div className="pj-vessel-header" style={{ flex: 1, minWidth: 0 }}>
            ...
            {editingTag ? (
              ...
              vessel.vessel_tag && (
                <span
                  ...
                  className="pj-vessel-title pj-editable"
                >
                  {vessel.vessel_tag} —{' '}
                </span>
              )
          ...
          {vessel.description && <p className="pj-vessel-description">{vessel.description}</p>}
        </div>
                ...
                {(Object.keys(VESSEL_STATUS_LABELS) as VesselStatus[]).map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{VESSEL_STATUS_LABELS[s]}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {modelerMenuOpen && (
              <div className="pj-dropdown-menu">
                <button
                  onClick={() => {
                    setModelerMenuOpen(false);
                    navigate(`/vessel-modeler?project=${projectId}&vessel=${vesselId}`);
                  }}
                    ...
                    {linkedModels.map((m) => (
                      <button
                        ...
                        onClick={() => {
                          setModelerMenuOpen(false);
                          navigate(
                            `/vessel-modeler?project=${projectId}&vessel=${vesselId}&model=${m.id}`
                          );
                        }}
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                          ...
                          {m.model_type && (
                            <span
                              style={{ fontSize: '10px', color: 'var(--clean-text-quaternary)' }}
                            >
                              {m.model_type}
                            </span>
                          )}
      ...
      <div className="pj-content">
        ...
        {/* Coverage vs scope — targets, achieved, and the read-only 3D view.
                        Collapsed by default: expanding it is what loads the 3D engine. */}
        <div className="pj-section-label">Coverage</div>
        <VesselCoverageScopeSection projectId={projectId!} projectVesselId={vesselId!} />
