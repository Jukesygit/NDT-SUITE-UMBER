...
import { useAvatarUrls } from '../../hooks/queries/useAvatarUrls';
import { avatarUrlFor } from '../../services/avatar-service';
  ...
  return (
    ...
    >
      <div className={`pm-sort-header ${align === 'center' ? 'pm-sort-header--center' : ''}`}>
        {label}
        {isActive && (
          <span className="pm-sort-indicator">{sortDirection === 'asc' ? '\u25B2' : '\u25BC'}</span>
        )}
      </div>
    </th>
  );
...
export function PersonnelTable({
  ...
  // One signing round trip for the whole page of people, not one per row.
  // Handles both stored shapes (bucket path, legacy public URL); a person whose
  // avatar cannot be resolved falls through to initials.
  const avatarUrlsQuery = useAvatarUrls(personnel.map((p) => p.avatar_url));
                  ...
                  >
                    <td className="pm-table td">
                      <div className="pm-person-cell">
                        <PersonAvatar
                          avatarUrl={avatarUrlFor(avatarUrlsQuery.data, person.avatar_url)}
                    ...
                    <td className="pm-table td">{person.organizations?.name || 'Unknown'}</td>
                    <td className="pm-table td">
