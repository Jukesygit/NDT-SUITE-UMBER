...
async function createOrUpdateProfile(employeeData) {
  const { name, position, email, mobile, address, startDate, dateOfBirth } = employeeData;
  ...
  if (profile) {
    ...
    const updates = {
      ...
      start_date: startDate || profile.start_date
    };
    ...
    const newProfile = {
      ...
      full_name: name,
      email: email && email !== 'N/A' ? email : `${name.toLowerCase().replace(' ', '.')}@matrixinspection.com`,
      position: position,
      phone: mobile && mobile !== 'N/A' ? mobile : null,
      address: address && address !== 'N/A' ? address : null,
...
async function importData(dryRun = true) {
  ...
  if (!dryRun && competenciesToInsert.length > 0) {
    ...
    console.log('\n✅ To perform the actual import, run: node scripts/import-complete-data.js --live');
  }
}
